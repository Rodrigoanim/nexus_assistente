...
def show_crud():
    ...
    with open(DB_PATH, "rb") as db_file:
        if st.download_button(
            ...
            # Registra o download na tabela log_acessos
            try:
                ...
                now = datetime.now()
                data_acesso = now.strftime('%Y-%m-%d')
                hora_acesso = now.strftime('%H:%M:%S')
                
                # Obtém user_id do session_state
                ...
                VALUES (?, ?, ?, ?, ?)
                """, (user_id, data_acesso, hora_acesso, "CRUD", "DOWNLOAD_DB"))
                
                conn.commit()
                conn.close()
                
            except Exception as e:
                st.error(f"Erro ao registrar download: {str(e)}")
                if 'conn' in locals():
                    conn.close()
